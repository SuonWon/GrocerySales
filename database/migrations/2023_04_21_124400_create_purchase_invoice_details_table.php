...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('purchase_invoice_details', function (Blueprint $table) {
            ...
            $table->foreign('InvoiceNo')->references('InvoiceNo')->on('purchase_invoices');
            $table->string('WarehouseNo',10);
            $table->foreign('WarehouseNo')->references('WarehouseCode')->on('warehouses');
            $table->string('ItemCode',10);
            $table->foreign('ItemCode')->references('ItemCode')->on('items');
            $table->decimal('Quantity',10,2);
            $table->decimal('NewQuantity',10)->default(0);
            ...
            $table->decimal('ExtraViss',10,3)->default(0);
